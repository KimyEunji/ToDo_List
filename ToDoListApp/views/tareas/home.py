...
from views.tareas.crear_tarea import mostrar_crear_tarea
from db.operations import buscar_tarea
from PIL import Image, ImageTk

def mostrar_home_tareas(w:tk.Tk):
   ...
   # Sutitulo y crear nuevo socio 
    tk.Label(text="Tareas", font=TITLES, bg=PRIMARY_COLOR, fg=SECONDARY_COLOR).grid(column=0, row=2, sticky="n", columnspan=7)
    tk.Button(text="Crear Tarea", font=TEXTS, bg=THIRD_COLOR, fg=SECONDARY_COLOR, command=lambda:mostrar_crear_tarea(w), relief="groove").grid(column=4, row=2)
